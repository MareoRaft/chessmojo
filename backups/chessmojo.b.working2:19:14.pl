...
sub defineSVG{
	reverseBoard();
		...
		for my $x (1..8){ for my $y (0,9){ $rowandcollabels->text( 'x'=>($x+0.25), 'y'=>($y+0.84), 'onclick'=>"changerect(evt)" )->cdata($letters[$x-1]) } }
...
%###################################### JAVASCRIPT #######################################
		%= javascript begin
          ...
          var json = JSON.parse(event.data);
					...
					document.getElementById( json.id ).style = "fill:blue"; %#works in firefox!!

...
sub defineSVG{
	reverseBoard('y');
		...
		my $whitesquares = $svg->group( 'fill'=>'yellow', 'opacity'=>0.7 );
		my $blacksquares = $svg->group( 'fill'=>'red', 'opacity'=>0.7 );
...
print "before\n";
	...
	if(0){
#	if( defined $lastclickedpiece && $coord ~~ ($lastclickedpiece->options) ){
#		# if so, move the piece and switch turn
#		$lastclickedpiece->setCoord($coord);
#		$turn->switchColor;
#		@$hash{qw(command ID x y)} = ('movepiece', $lastclickedpiece->ID, $lastclickedpiece->X, 9-$lastclickedpiece->Y);
#		$$hash{'message'} = 'good option selected';
	}
	...
	#2. if coord is piece of whose turn it is, displacy options
		...
		# if so, move the piece and switch turn
		if( my $enemy = coordToPiece($coord) ){
			$enemy->setCoord([0,0]);
			$$hash{'IDtoremove'} = $enemy->ID
		}
		$lastclickedpiece->setCoord($coord);
		$turn->switchColor;
		@$hash{qw(command ID x y lastclickedoptions)} = ('movepiece', $lastclickedpiece->ID, $lastclickedpiece->X, 9-$lastclickedpiece->Y, $lastclickedpiece->optionIDs);
	}

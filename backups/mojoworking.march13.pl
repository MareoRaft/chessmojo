...
sub definePieceAndEnemy{
	my ($x,$y,%moves) = @_;
	push @pieces, piece->new( %moves, 'color'=>'white', 'coord'=>[$x,$y] );
	push @pieces, piece->new( %moves, 'color'=>'black', 'coord'=>[$x,9-$y] );
}
...

{
	...
	sub svgOfPiece{ #this function is for use with SVG.pm stuff.  ex: $group->svgOfPiece($piece)
		...
		my $svg = $root->svg( 'ID'=>$obj->ID, 'x'=>$obj->X, 'y'=>$obj->Y, 'class'=>$obj->color, %moves );
...
sub makeMove{
	...
	my %hash;
		if( !defined $move->pieceToRemove and $piece->isAnEnemyOn($move->toCoord) ){
			my $piecetoremove = deletePieceOnCoord($move->toCoord);
		...
		# to javascript
			@hash{qw(command ID x y message)} = ('movepiece', $piece->ID, $move->toX, $move->toY, $move->humanNotation);
			$hash{'IDtoremove'} = $move->pieceToRemove->ID if defined $move->pieceToRemove;

	if( $piece->type eq 'king' ){ $piece::cancastle{$piece->color}{'left'} = $piece::cancastle{$piece->color}{'right'} = 0 }
	elsif( $piece->type eq 'rook' ){
		if( $piece->X == 1 ){ $piece::cancastle{$piece->color}{'left'} = 0 }
		elsif( $piece->X == 8 ){ $piece::cancastle{$piece->color}{'right'} = 0 }
	}

	if( defined $move->coMove ){
		my %comovehash = makeMove( $move->coMove );
		$self->send({json => \%comovehash});
	}
	else{ $turn->switchColor }
...
sub squareClicked{
	...
	if( $piece = coordToPiece($coord) and $piece->myTurn ){
		@hash{qw(command moves)} = ('fillsquares', $piece->moveIDs); #print 'yo'; my $l = $piece->moves; print $l; print @{$piece->moveIDs}; #foreach (@{$piece->moves}){ print $_->humanNotation };
	...
	elsif( defined $lcpiece and $lcpiece->myTurn and my $move = piece::isEl($coord,$lcpiece->moves) ){
		%hash = makeMove( $move );
	}
...
$\ = "\n";
my $address = $self->tx->remote_address; print "address is $address";
my $port    = $self->tx->remote_port; print "port is $port";

#	if( $self is not element of @selves ){
		push @selves, $self; $" = ', '; print @selves;
#	}
				...
				$self->on(
					json => sub {
						...
						my $se; for $se (@selves){ $se->send({json => $hash}); }
						}
				);
...
get '/style' => sub {
  my $self = shift;
  $self->stash( whitesquares=>"#4dffff", blacksquares=>"#4d4dff" );
  $self->res->headers->cache_control('max-age=1, no-cache');

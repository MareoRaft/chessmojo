...
sub new{
	...
	my %options = @_;
	my $uniqueID = '';
	if( exists $options{'room'} ){ # needed
		my $room = $options{'room'};
		$uniqueID = $#{$room->piecesRef} + 1; # Each piece's ID is it's index in @pieces.
	}
	bless { 'ID'=>'piece'.$uniqueID, 'type'=>'queen', 'coord'=>[0,0], 'color'=>'white', %options } => $packagename
}
...
sub myTurn{
	my $piece = shift;
	if( $piece->color eq $piece->room->turn->color ){ return 1 }
	return 0
}
...
sub isAPieceOn{
	my $piece = shift;
	my ($coord) = @_;
	foreach ($piece->room->pieces){ if( @$coord ~~ @{$_->coord} ){ return $_ } }
	return 0
}
...
sub isAnEnemyOn{
	my $piece = shift;
	my ($coord) = @_;
	foreach ($piece->room->pieces){ if(   @$coord ~~ @{$_->coord}   &&   $piece->color ne $_->color   ){ return $_ } }
	return 0
}
...
sub isAFriendOn{
	my $piece = shift;
	my ($coord) = @_;
	foreach ($piece->room->pieces){ if(   @$coord ~~ @{$_->coord}   &&   $piece->color eq $_->color   ){ return $_ } }
	return 0
}
...
sub isCoordInCheck{ # even though we are looking at a coord, we need to know in check by WHICH SIDE, therefore the $piece->color.
	my $piece = shift;
	my ($coord) = @_;
	foreach ($piece->room->pieces){ if( $_->color ne $piece->color and isEl($coord,$_->moves('nocastle'))){ return 1 } } # must exclude castle moves to prevent infinite loop
	return 0
}
...
sub isValidEnPassant{ # takes in the space where we would find the ENEMY
	...
	if( my $enemy = $piece->isAnEnemyOn($coordofenemy) ){
		my $lastmove = ${$piece->room->movesRef}[-1];
		# we can check for object equality like so: (use Scalar::Util)
		if( refaddr($lastmove->piece)==refaddr($enemy) and $lastmove->type eq 'jump' ){ return $enemy }
	}
	return 0
}
...
sub qualifyLineOfMoves{
	...
	for( my $i=0; $i<=$#lineofmoves; ++$i ){ #print "color is ",$piece->color," and coord is ",${$piece->coord}[0],',',${$piece->coord}[1]; print "linemoves are "; foreach(@lineofmoves){print "[$$_[0],$$_[1]]" } print " i is $i.\n";
		if( $lineofmoves[$i] ~~ $piece->coord ){ $state = 'after' }
...
sub pawnMoves{ # for white pawns specifically
	my $piece = shift;
	my @moves; my $enemy;
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[$piece->X,$piece->Y+1], 'type'=>'step' ) if !$piece->isAPieceOn( [ $piece->X, $piece->Y+1 ] );
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[$piece->X,$piece->Y+2], 'type'=>'jump' ) if $piece->Y==2 and !$piece->isAPieceOn( [ $piece->X, $piece->Y+2 ] ) and !$piece->isAPieceOn( [ $piece->X, $piece->Y+1 ] );#jump 2 first move only
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[ $piece->X+1, $piece->Y+1 ] ) if $piece->isAnEnemyOn( [$piece->X+1,$piece->Y+1] ); #capture diagonally right
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[ $piece->X-1, $piece->Y+1 ] ) if $piece->isAnEnemyOn( [$piece->X-1,$piece->Y+1] ); #capture diaganolly left
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[$piece->X+1,$piece->Y+1], 'piecetoremove'=>$enemy, 'type'=>'enpassant' ) if $enemy = $piece->isValidEnPassant([$piece->X+1,$piece->Y]);
		push @moves, move->new( 'piece'=>$piece, 'tocoord'=>[$piece->X-1,$piece->Y+1], 'piecetoremove'=>$enemy, 'type'=>'enpassant' ) if $enemy = $piece->isValidEnPassant([$piece->X-1,$piece->Y]);
	foreach my $move (@moves){ $move->checkForPromotion }
	return @moves
}

sub canCastleLeft{ # we will disqualify castling at the moment of king or rook movement, not in this function
	my $piece = shift;
	if( $piece->room->canCastle($piece->color,'left') ){
		for my $x (2..4){ if( $piece->isAPieceOn([$x,1]) ){ return 0 } }
		for my $x (1..5){ if( $piece->isCoordInCheck([$x,1]) ){ return 0 } }
		return $piece->room->coordToPiece([1,1])
	}
	return 0
}
...
sub canCastleRight{ # we will disqualify castling at the moment of king or rook movement, not in this function
	my $piece = shift;
	if( $piece->room->canCastle($piece->color,'right') ){
		for my $x (6..7){ if( $piece->isAPieceOn([$x,1]) ){ return 0 } }
		for my $x (5..8){ if( $piece->isCoordInCheck([$x,1]) ){ return 0 } }
		return $piece->room->coordToPiece([8,1])
	}
	return 0
}
...
sub castleMoves{ # for white kings specifically
	my $king = shift;
	my @moves;
		if( my $rook = $king->canCastleLeft ){
			my $castlemove = move->new( 'piece'=>$rook, 'tocoord'=>[4,1] );
			push @moves, move->new( 'piece'=>$king, 'tocoord'=>[3,1], 'type'=>'castle', 'comove'=>$castlemove )
		}
		if( my $rook = $king->canCastleRight ){
			my $castlemove = move->new( 'piece'=>$rook, 'tocoord'=>[6,1] );
			push @moves, move->new( 'piece'=>$king, 'tocoord'=>[7,1], 'type'=>'castle', 'comove'=>$castlemove )
		}
	return @moves
}
...
sub whiteMoves{
	...
	my @moves; my @lineofmoves; my @individualmoves;
		...
		if( $piece->type eq 'bishop' || $piece->type eq 'queen' ){ #IF, not ELSIF
			...
			@moves = $piece->qualifyIndividualMoves(@individualmoves);
			push @moves, $piece->castleMoves unless defined $nocastle;
		}
	#print "\n moves before: @moves\n\n";
...
sub moves{
	...
	if( $piece->isOnBoard ){
		if( $piece->color eq 'white' ){
			...
			$piece->room->reverseBoard('y'); @moves = reverseMoves($piece->whiteMoves($nocastle)); $piece->room->reverseBoard('y')
		}
	}
	return @moves
}
...
sub stringify{
	my $piece = shift;
	my $toprint = 'ID: '.$piece->ID.'. A '.$piece->color.' '.$piece->type.' has position ['.$piece->X.','.$piece->Y.']. And '.($piece->isOnBoard? 'is': 'is not')." on the board.";
	#$toprint .= "MOVES:\n";

...
sub new{
	...
	my %options = @_;
	my $room = bless {
		'controllers'=>[], # these are mojolicious controllers, one for each websocket connection
		'pieces'=>[],
		'moves'=>[],
		'cancastle'=>{
...
sub pushControllers{
	my $room = shift;
	my @controllers = @_;
	push @{$room->controllersRef}, @controllers;
}
sub setLCPiece{
	my $room = shift;
	my ($lcpiece) = @_;
...
sub squareClicked{
	...
	if( $ball{'color'} ne $room->turn->color ){
		@ball{qw(command message)} = ('donothing', 'It is '.$room->turn->color."'s turn and you are $ball{'color'}");
		return %ball;
	}
	#1. if coord is piece of whose turn it is, display moves
	if( $piece = $room->coordToPiece($coord) and $piece->myTurn ){
